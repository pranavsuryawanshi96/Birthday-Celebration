...
function App() {
  ...
  const page4Ref = useRef(null); // Random Compliment Generator
  const page5Ref = useRef(null); // Gallery
  ...
  const goToPage = (pageNumber) => {
    ...
    if (pageNumber === currentPage || !currentPageRef?.current || !nextPageRef?.current) {
      return;
    }
    ...
    <div className="app">
      ...
      >
        <section className="hero">
          <h1 id="heroTitle">
            {birthdayReached ? (
              <>
                Happy Birthday <span className="highlight">Tanishka</span> 🎂
              </>
            ) : (
              <>
                Counting down to <span className="highlight">Tanishka's</span>
                special day 🎂
              </>
            )}
          </h1>
      ...
      >
        ...
        <button className="page-nav-btn" onClick={() => goToPage(4)}>
          🎮 Start Your Birthday Quest
        </button>
      </div>

      {/* PAGE 4: Random Compliment Generator */}
      <div
        ref={page4Ref}
        className={`page content-page ${currentPage === 4 ? "active" : ""}`}
      ...
      <div
        ref={page5Ref}
        className={`page content-page ${currentPage === 5 ? "active" : ""}`}
        style={{ visibility: currentPage === 5 ? "visible" : "hidden" }}
      >
        <button className="back-btn" onClick={() => goToPage(4)}>
          ← Back
        </button>
        <Gallery isActive={currentPage === 5} />
